...
The style panel's border Color and Width inputs shared a field name, so a saved border kept only the width and reopening the block's Style Settings threw before the panel appeared. Each input now has its own name, and blocks saved with the old collision open their panel again. [#10](https://github.com/owen2345/camaleon_editor/pull/10).
